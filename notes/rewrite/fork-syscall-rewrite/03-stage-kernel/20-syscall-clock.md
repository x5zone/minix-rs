...
| # | 决策 | 选项 | 结论 | 理由 |
|---|------|------|------|------|
| D1 | SETALARM 定时器 | `minix_timer_t` 链表 vs BTreeMap | **BTreeMap** | 与 14-clock-timer D2 一致 |
| D2 | cause_alarm 回调 | 函数指针 vs TimerAction | **TimerAction::NotifyAlarm** | 与 14-clock-timer D6 一致 |
...
4. **`ClockState::set_timer/reset_timer`**：设置新定时器或取消旧定时器，与 14-clock-timer D2 的 `BTreeMap` 定时器管理对接。
...
- [14-clock-timer.md](14-clock-timer.md) — ClockState, TimerAction, vtimer_check
