...
### 1.2 系统调用 vs IPC：同一入口的两种语义

用户态执行 `int 0x80`/`syscall`/`svc` 触发 trap（详见 14），trap 入口根据 `m_type` 将控制流分到两条路径：

| 维度 | IPC（do_ipc） | 系统调用（kernel_call） |
|------|--------------|----------------------|
| **本质** | 进程间消息中转，内核不参与语义 | 进程请求内核执行特权操作 |
| **入口判断** | `m_type < KERNEL_CALL`（0x600） | `m_type >= KERNEL_CALL` |
| **路由函数** | `do_ipc(call_nr, ...)` | `kernel_call_dispatch(caller, msg)` |
...
**关键差异**：IPC handler 本身**就是内核**——内核构造一条消息转发给目标进程；系统调用 handler 是**内核服务**——内核直接完成操作（fork/exec/vmctl 等）。两者共享 `sys_call` 入口但语义正交，互不调用对方 handler。

### 1.3 入口路径与 KERNEL_CALL 归一化

```
用户态: sys_call(SYS_VMCTL, ...)
  → trap (int 0x80 / syscall / svc)
  → 14 trap 入口（汇编保存上下文 + 判别 m_type）
  → if m_type >= KERNEL_CALL (0x600):
      call_nr = m_type - KERNEL_CALL   ← 归一化为 0..57 索引
      ...
      → do_ipc(call_nr, r2, r3)  ← 12-ipc-core
```
...
| **D6** | `#[cfg(target_arch)]` 条件注册 handler | `dispatch_arch_*` stub 返回 `BadCall` | 避免行为选择泄漏到编译期（模式 14） |
...
C 用 `#if defined(__i386__)` 条件注册 x86 特有 syscall。Rust 将所有 syscall 编入 `enum Syscall`（包括 `Devio`、`Padconf` 等），架构不支持时通过 `dispatch_arch_*` stub 返回 `BadCall`。

**为什么不直接 `#[cfg(target_arch)]` 排除变体**？这会让 `enum Syscall` 在不同架构上有不同变体集——`match` 的穷尽检查失效，且违反硬件抽象原则（模式 14：行为选择泄漏到编译期）。

**实现**（[syscall.rs:327-355](file:///home/xzhao/github/minix-rs/os/kernel/src/syscall.rs#L327-L355)）：
...
`#[cfg]` 在此处仅选**实现**（stub vs 真实），不选**行为**——所有架构上 `dispatch_arch_devio` 都是合法的 dispatch 路径，只是返回值不同。真实现位于 [syscall.rs:1167-1218](file:///home/xzhao/github/minix-rs/os/kernel/src/syscall.rs#L1167-L1218)（x86_64）等位置。
...
**三段式分派对齐 C**：边界检查（`TryFrom`） → 权限检查（`kcall_filter_check`） → 路由（`match`）。`kcall_filter_check` 定义在 `ipc_filter.rs`（详见 [23-ipc-filter](23-ipc-filter.md) §4.1），将 `[u32; 2]` 合并为 `u64` 后按位测试。
...
| 测试 | 验证 | 实现位置 |
|------|------|---------|
| `dispatch_schedule` 拒绝非 SYS_PROC 调用方 | 权限路径 EPERM | syscall.rs:1379-1388 |
| `dispatch_getmcontext` 无效 endpoint → EINVAL | 边界检查 | syscall.rs:1413-1424 |
...
| 测试点 | 验证 | 实现位置 |
|--------|------|---------|
| `dispatch_arch_devio` 非 x86_64 → `BadCall` | D6 stub 路径 | syscall.rs:327-330 |
| `dispatch_arch_padconf` 非 ARM → `BadCall` | D6 stub 路径 | syscall.rs:352-355 |
...
**取消的改进（避免过度设计）**：
- E5（dispatch_arch_* stub 用 macro）：评估后决定不实施——6 个 stub 虽重复 1 行但签名不同，macro 隐藏签名差异，抽象代价 > 重复代价。
