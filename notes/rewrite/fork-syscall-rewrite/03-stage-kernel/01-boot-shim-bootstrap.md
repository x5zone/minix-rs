...
> |---------|--------|----------|---------|
> | 1. 临时切除（pre_init） | `pg_utils.c:32 cut_memmap()`（由 `pre_init.c` 调用）切掉 boot module 占用的物理内存 | `build_memmap()` 跳过 module 区域（`os/boot-shim/src/opensbi_helpers.rs:375`） | 本文档 §2.4（本节上下文） |
