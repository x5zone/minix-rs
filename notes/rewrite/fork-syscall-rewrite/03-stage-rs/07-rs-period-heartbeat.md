...
- **`another_initializing`/`is_updating` 参数注入**：C 靠全局表查询（`lookup_slot_by_flags(RS_INITIALIZING)`，request.c:1013）与 `SRV_IS_UPDATING`（const.h:114）——Rust 把这两个判定结果作为布尔参数传入，`period_decision` 不触表（保持纯函数）。喂参来源：`RProcTable::lookup_by_flags(RFlags::INITIALIZING)`（process_table.rs，R32 已在函数文档互链）。
