...
6. `add` 标志传播：`INCLUDES_VM` 条目插入后全链 `lu_flags`/`init_flags` 含该位；`vm()` 指针指向首个。
7. `end_update_role`：四角色（`initializing` 真/假两族的 4 组合）。
8. `abort_action`：四阶段分派。
9. `default_prepare_maxtime`：0 → 默认值；非 0 → 原值。
...
- `notes/rewrite/fork-syscall-rewrite/03-stage-rs/02-rs-process-table.md` —— `RupdateDescriptor`/`RupdateFlags` 数据形状
