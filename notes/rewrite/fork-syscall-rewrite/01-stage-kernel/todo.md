...
| D-18 | IPC 过滤 | `IPCF_EL_MATCH` 宏链（ipc_filter.h:19-41） | 23 §4.5/Ch6 [P2] | **✅ 已解决（2026-09-06，随 D-16）**：`el_match`（MATCH_M_TYPE 相等 + MATCH_M_SOURCE 相等或 ANY_USR/SYS/TSK 类别匹配，类别经引擎注入的解析器：nr≤0=Task / 有 SYS_PROC=Sys / 否则 Usr）+ `el_check`（设置期元素合法性：至少一个 MATCH 标志；MATCH_M_SOURCE 须 ANY_* 或 isokendpt 可解析，经 proc_table 解析）。见 `ipc_filter::chain_allowed`/`el_match`/`el_check` |
