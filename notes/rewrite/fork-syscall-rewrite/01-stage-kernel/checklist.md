...
> **2026-08-12 更新**: 新增 28-30 文档覆盖 `usermapped_data.c` / `debug.c` / `profile.c` 三个非 syscall C 源文件（见 §4.11）。模块级 C 符号覆盖率从 31.2% (408/1306) 提升至 33.8% (441/1306)；语义缺口 `Gaps (semantic): 0` — 三个文档作用域内无遗漏。剩余 ~863 缺口主要为 `arch/earm/bsp/ti/omap_*` BSP 寄存器定义 (286+) 和 `arch/i386/` 历史架构代码 (210+)，已以 ARCH-stub 形式分散在各 doc 内，无需独立文档。
...
| F-03 | `do_clear` | do_clear.c:24-78 | `syscall_process.rs:392` `dispatch_clear` | ⚠️ Partial→Partial+++ | ✅ (2026-06-14) 核心清理; **✅ (2026-06-16) 修复 P0 语义漂移**: 旧代码操作 caller 而非 target (PM 调 SYS_CLEAR 会把自己标 SLOT_FREE!), 现在正确操作 target + endpoint 验证 + isemptyp 提前返回 + PrivTable 传入 + SYS_PROC privilege slot 释放 (s_proc_nr=None). **✅ (2026-08-01) IRQ/timer 落地**: 全局 `irq_manager()` 遍历 NR_IRQ_HOOKS 移除 owner==target 的钩子 + `clock_state.reset_timer(timer_id)` 取消 s_alarm_timer (新增 clock_state 参数 + init_irq_manager_for_test 测试 helper). DEFERRED: release_address_space (VM), clear_endpoint (IPC) |
...
| F-24 | `do_vdevio` | do_vdevio.c:24-165 | `syscall_device.rs:419` | ⚠️ Partial | 参数提取 + 类型/方向解析 + vec_size 校验 + **ENOSYS 返回 (2026-06-16: 语义漂移修复, OK→ENOSYS)**; `CurrentArchSyscall::dispatch_vdevio` (X86_64Syscall) 已接线; 批量 I/O 执行 DEFERRED (需 data_copy_vmcheck) |
...
| # | C Handler | C File:Line | Rust 实现 | 状态 | 备注 |
|---|-----------|-------------|-----------|------|------|
| F-36 | `do_getinfo` | do_getinfo.c:20-228 | `misc.rs:666` `dispatch_getinfo` | ⚠️ Partial→Partial+ | GET_WHOAMI ✅; GET_KINFO ✅ (2026-06-14: user_sp/freepde_start/vir_kern_start 从 kernel_info() 获取真实值); GET_PROC/GET_PROC2 ✅ (2026-06-16: SELF替换+isokendpt endpoint验证, data_copy_vmcheck仍DEFERRED); 通配分支 ✅ (返回 ENOSYS); ProcTab/PrivTab/LoadInfo 仍 ENOSYS (需 data_copy_vmcheck) |
| F-37 | `do_trace` | do_trace.c:20-208 | `misc.rs:1223` `dispatch_trace` | ✅ Complete (2026-08-01): 13/13 request 全部实现 | ✅ 13 个 request 全部覆盖: 5 flag 操作 + 6 跨地址空间拷贝 (data_copy_vmcheck + VmSuspend) + T_GETUSER (proc-struct + priv-struct 双分支, 经 ProcInfoStruct/PrivInfoStruct 快照 + read_word_at_offset) + T_SETUSER (CpuContextArch::write_user_register, 三架构段寄存器保护 + PSW 掩码). `dispatch_trace` 签名: `&mut ProcessTable` + `&PrivTable`. 无 DEFERRED |
| F-38 | `do_update` | do_update.c:20-340 | `misc.rs:1553` `dispatch_update` | ✅ Complete (2026-08-01): 12/12 步全部实现 | ✅ 7 步验证 (endpoint/SYS_PROC/updatable/src!=dst) + inherit_priv_irq/io/mem (KPriv::add_irq/add_io/add_mem, dedup+CHECK_* flag) + s_ipc_to OR 合并 + abort_proc_ipc_send (SYS_UPD_ROLLBACK: RTS_SENDING clear + SenderQueue::remove_by_nr) + slot swap (ProcessTable::swap_slots + PrivTable::swap_slots, core::mem::swap + split_at_mut) + adjust_proc_slot (endpoint/nr/priv_id/caller_q/scheduler/cpu/cpu_mask 恢复, caller_q via mem::replace) + adjust_priv_slot (s_id/s_proc_nr/pending bits/diag_sig 恢复). swap_proc_slot_pointer (ptproc) + swap_memreq no-op (两进程非 runnable). adjust_asyn_table 跳过 (C 非致命, 需 data_copy). `priv_table: &mut PrivTable`. 4 个测试 |
| F-39 | `do_sprofile` | do_sprofile.c:20-132 | `misc.rs:1898` `dispatch_profile` | ⚠️ Partial (2026-06-16): 4/8 步实现 | ✅ (2026-06-16) **状态机实现**: 新增 `SPROFILING: AtomicBool` 全局 (C `int sprofiling`); PROF_START 用 `compare_exchange(false→true)` 原子地 check-and-set, 已经在跑 → EBUSY; PROF_STOP 用 `compare_exchange(true→false)` 原子地 check-and-clear, 没在跑 → EBUSY; validation 失败自动 rollback (不污染 future PROF_START). **测试基础设施**: 新增 `SPROF_TEST_LOCK: AtomicBool` 自旋锁 (no_std 兼容, 替代 std::sync::Mutex) + `sprof_test_setup()`/`teardown()` 串行化并行 cargo test 之间的状态. 9 个测试覆盖 (5 个原有 + 4 个新: double-start EBUSY / stop-without-start EBUSY / start-rollback / stop-after-start ENOSYS). DEFERRED: init_profile_clock (PROF_RTC, P0-08) + nmi_watchdog_start/stop (PROF_NMI, P0-15 arch-abstractions) + data_copy sprof_info + data_copy sample buffer (P1-05 Direct Map) + clean_seen_flag (MF_SPROF_SEEN) |
...
| F-43 | `do_getmcontext` | do_mcontext.c:23-60 | `syscall.rs:1080` `dispatch_getmcontext` | ✅ Done (2026-08-01) | 64-bit 完整实现: endpoint 校验 (EINVAL) + is_kernel (EPERM) + `data_copy_vmcheck` 拷 mcontext (user→kernel) + `SignalContext::mcontext_clear_flags` 清 mc_flags + `data_copy_vmcheck` 拷回 (kernel→user). 关键洞察: C `do_mcontext.c` 的 FPU 拷贝是 `#if defined(__i386__)` only — 64-bit 不存在, 内核用 lazy FPU. 新增 `type Mcontext` + `fn mcontext_clear_flags` 到 `SignalContext` trait (arch/signal_context.rs:241-246); 三架构 `#[repr(C)]` mcontext: x86_64 (824B, gregs[21]+fpregs[640]+tlsbase+mc_magic+mc_flags), arm64 (816B, gregs[32]+sp+pc+pstate+fpregs[528]+mc_magic+mc_flags), riscv64 (544B, gregs[32]+sepc+sstatus+fpregs[264]+mc_magic+mc_flags). 3 个测试覆盖 (invalid endpoint / kernel target EPERM / user target VmSuspend — 页缺失 on unmapped ctx_ptr, 旧 ENOSYS 期望已更新) |
| F-44 | `do_setmcontext` | do_mcontext.c:62-106 | `syscall.rs:1188` `dispatch_setmcontext` | ✅ Done (2026-08-01) | 64-bit 完整实现: endpoint 校验 (EINVAL) + `data_copy_vmcheck` 拷 mcontext (user→kernel) + 返回 OK. 关键设计: C **不**检查 iskerneln (kernel 进程可在 context switch 时恢复 FPU 状态); 64-bit 无 FPU 拷贝 (`#if defined(__i386__)` only). 复用 `SignalContext::Mcontext` 关联类型 (见 F-43). 3 个测试覆盖 (invalid endpoint / **kernel target 允许** VmSuspend / user target VmSuspend — 旧 ENOSYS 期望已更新) |
...
| F-47 | `do_iopenable` (x86) | do_iopenable.c | `syscall_device.rs:491` | ✅ Complete (2026-08-01) | SELF 解析 + endpoint 验证 + iskerneln 检查 + IOPL=3 修改 cpu_context.psw via `enable_user_io()`; &mut ProcessTable 接入; 5 个单元测试. trap frame 同步 DEFERRED 已解除: C `p_reg` = Rust `cpu_context`，无独立内核栈异常帧，syscall handler 上下文修改 cpu_context.psw 返回用户态时自动生效 |
...
### 4.11 Doc 28-30 — 非 syscall C 源码覆盖（2026-08-12 新增）

> **触发**: Task 1 覆盖率扫描发现 `usermapped_data.c` / `debug.c` / `profile.c` 三个非 `do_*.c` 源文件未被任何文档完整覆盖。新建 28-30 文档补足。
> **验证**: `coverage-extract.py` 三文档均 `Gaps (semantic): 0`（见 `.review/trae/fork-syscall-rewrite/scans/{28,29,30}-*-glm-SYMBOLS-verify.md`）。

| # | Doc | C 源文件 | C 符号数 | 文档覆盖 | Rust 覆盖 | 状态 | 决策 |
|---|-----|---------|---------|---------|----------|------|------|
| F-50 | 28-usermapped-data.md | `usermapped_data.c` + `arch/i386/usermapped_data_arch.c` | 11 全局变量 + 3 IPC vecs | 10/42 semantic | 3/42 semantic | ✅ WONTFIX | 64-bit 重写不保留 `.usermapped` 段；数据通过 `sys_getinfo` 获取（D1 决策） |
| F-51 | 29-kernel-debug.md | `debug.c` | 17 函数 + 4 宏 | 39/74 semantic | 0/74 semantic | ✅ Partial+ | **✅ runqueues_ok / print_proc / rtsflagstr 已实现 (Phase 8, 2026-08-13)** — `debug.rs` 提供 `write_rts_flags` / `write_misc_flags` / `print_proc` / `runqueues_ok`; BKL timing 用 `debug_assert!` + `log!` 替代；非生产路径 |
| F-52 | 30-kernel-profile.md | `profile.c` | 4 函数（含 1 partial） | 25/56 semantic | 4/56 semantic | ✅ Partial+ | profile clock interface 保留；**✅ sample collection 已实现 (Phase 8, 2026-08-13)** — `profile_sample` + `profile_clock_handler` + `SprofSample`/`SprofProc` + `ack_profile_clock` trait; NMI profiling WONTFIX（D2/D3/D4 决策） |
...
| 2 | SYS_CLEAR | `syscall_process.rs:392` | ⚠️ Partial→Partial+++ (2026-08-01) | ✅ **(2026-06-16) 修复 P0 语义漂移**: 1) 旧代码操作 caller (PM) 而非 target — PM 调用 SYS_CLEAR 会把自己标记 SLOT_FREE! 现在正确操作 target; 2) 新增 endpoint 验证 (isokendpt → EINVAL); 3) 新增 isemptyp 提前返回; 4) 传入 ProcessTable + PrivTable, 实现 SYS_PROC privilege slot 释放 (s_proc_nr = None); 5) **✅ (2026-08-01) IRQ hooks + alarm timer 落地**: 全局 `irq_manager()` 遍历移除 owner==target 的钩子 + `clock_state.reset_timer(timer_id)` 取消 s_alarm_timer; 6) DEFERRED: release_address_space (VM) / clear_endpoint (IPC) |
...
| 17 | SYS_UMAP_REMOTE | `syscall_copy.rs:298` | ✅ Complete (2026-08-01) | endpoint 验证 + umap_common_impl + data_copy_vmcheck; VMSUSPEND 恢复 |
| 18 | SYS_VUMAP | `syscall_copy.rs:338` | ✅ Complete (2026-08-01) | vectored umap + per-entry data_copy_vmcheck; VMSUSPEND 恢复 |
| 19 | SYS_IRQCTL | `syscall_device.rs:174` | ✅ Done (2026-06-16) | SetPolicy/RmPolicy/Enable/Disable; generic_handler DEFERRED |
| 20 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 21 | SYS_DEVIO | `syscall_device.rs:325` | ✅ Implemented (2026-06-16) | PortIo trait 迁移至 minix_plat, x86_64 X86_64PortIo (in/out inline asm), `CurrentArchSyscall::dispatch_devio` (X86_64Syscall) 接入 dispatch_devio |
| 22 | SYS_SDEVIO | `syscall.rs` `dispatch_sdevio` | ✅ Complete (2026-08-01) | 参数提取 + endpoint 验证 + type/direction 解析 + CHECK_IO_PORT + 对齐检查; x86_64 已接线; 批量 I/O data_copy_vmcheck + VMSUSPEND |
...
| 26 | SYS_GETINFO | `misc.rs:666` `dispatch_getinfo` | ⚠️ Partial | 19/19 GetInfoRequest 变体; GET_KINFO/KERNEL/WHOAMI/PROC/PROCTAB/PRIV/PRIVTAB/REGS/IRQHOOKS/IRQACTIDS/IDLETSC/RANDOMNESS/RANDOMNESS_BIN/LOADINFO/MACHINE/CPUINFO/HZ/CPUTICKS 已实现; GETIMAGE/MONPARAMS 部分实现; data_copy_vmcheck + VMSUSPEND |
| 27 | SYS_ABORT | `syscall.rs` `dispatch_abort` | ⚠️ Partial | 参数验证 + panic 路径; DEFERRED: 实际 abort 机制 |
| 28 | SYS_IOPENABLE | `syscall_device.rs:491` | ✅ Complete (2026-08-01) | SELF 解析 + IOPL=3 via `enable_user_io()`; trap frame DEFERRED 已解除 (C `p_reg` = Rust `cpu_context`) |
| 29 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 30 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 31 | SYS_SAFECOPYFROM | `syscall_copy.rs:216` | ✅ Complete (2026-08-01) | safecopy_common_impl + verify_grant + data_copy_vmcheck; VMSUSPEND 恢复 |
| 32 | SYS_SAFECOPYTO | `syscall_copy.rs:236` | ✅ Complete (2026-08-01) | 同 SAFECOPYFROM (WRITE 方向) |
| 33 | SYS_VSAFECOPY | `syscall_copy.rs:256` | ✅ Complete (2026-08-01) | vectored safecopy + per-entry data_copy_vmcheck; VMSUSPEND 恢复 |
| 34 | SYS_SETGRANT | `syscall.rs:436` | ✅ Fixed |
| 35 | SYS_READBIOS | `syscall.rs` `dispatch_readbios` | ⚠️ Partial (2026-08-01) | 参数提取 + BIOS 内存范围验证; x86_64 已接线; data_copy DEFERRED (需 virtual_copy_vmcheck) |
| 36 | SYS_SPROF | `misc.rs:1898` `dispatch_profile` | ⚠️ Partial | ✅ (2026-06-16): 输入验证(ProfAction/ProfIntrType+isokendpt); ✅ (2026-08-01): START PROF_RTC → ClockArch::init_profile_clock; STOP → stop_profile_clock; DEFERRED: sprofiling 状态 + data_copy; WONTFIX: PROF_NMI (NMI 子系统) |
| 37 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 38 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 39 | SYS_STIME | `syscall_clock.rs:314` | ✅ Implemented (2026-06-15) |
| 40 | SYS_SETTIME | `syscall_clock.rs:360` | ✅ Implemented (2026-06-15) |
| 41 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 42 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 43 | SYS_VMCTL | `syscall.rs` `dispatch_vmctl` | ⚠️ Partial (2026-08-13, FIX-24) | VmInhibitSet/Clear + ClearPageFault + BootInhibitClear + MemReqGet/MemReqReply + GetPdbr/FlushTlb/InvlPg (TlbArch 三架构) + ClearMapCache (WONTFIX no-op) 已实现 |
| 44 | SYS_DIAGCTL | `syscall.rs` `dispatch_diagctl` | ⚠️ Partial |
...
| 47 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 48 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 49 | *(unused)* | not in Rust | ❌ WONTFIX — reserved/unused |
| 50 | SYS_GETMCONTEXT | `syscall.rs:1080` | ✅ Done (2026-08-01) | 64-bit 完整: data_copy_vmcheck + mcontext_clear_flags; FPU 拷贝 `#if defined(__i386__)` only 不适用 |
| 51 | SYS_SETMCONTEXT | `syscall.rs:1188` | ✅ Done (2026-08-01) | 64-bit 完整: data_copy_vmcheck 拷 mcontext (user→kernel) + 返回 OK; 无 iskerneln 检查 (与 C 一致) |
| 52 | SYS_UPDATE | `misc.rs:1553` `dispatch_update` | ✅ Complete (2026-08-01) | 7 步验证 (endpoint/SYS_PROC/updatable/src!=dst) + inherit_priv_irq/io/mem (KPriv::add_*, dedup+CHECK_* flag) + s_ipc_to OR 合并 + abort_proc_ipc_send (SYS_UPD_ROLLBACK: RTS_SENDING clear + SenderQueue::remove_by_nr) + slot swap (ProcessTable::swap_slots + PrivTable::swap_slots, core::mem::swap + split_at_mut) + adjust_proc_slot/adjust_priv_slot (identity 字段恢复, caller_q via mem::replace). swap_proc_slot_pointer (ptproc) + swap_memreq no-op (非 runnable). adjust_asyn_table 跳过 (C 非致命). 4 测试 |
| 53 | SYS_EXIT | `syscall_process.rs:194` | ⚠️ Partial |
| 54 | SYS_SCHEDCTL | `syscall_process.rs:502` | ✅ Fixed (2026-06-17) |
| 55 | SYS_STATECTL | `syscall_process.rs:310` | ✅ Complete (2026-08-13, Phase 6) | ✅ (2026-06-14) SetStateTable+ClearIpcFilters+AddIpcBlFilter+AddIpcWlFilter implemented; **✅ (2026-08-13, Phase 6) ClearIpcRefs 落地** (调用 `clear_ipc_refs` 清 pending bits + 唤醒 blocked 进程); 签名扩展加 `proc_table`. 4 测试. DEFERRED: filter elements 的 `data_copy_vmcheck` 填充 (依赖 VM-side PTE walk) |
| 56 | SYS_SAFEMEMSET | `syscall_copy.rs:379` | ✅ Complete (2026-08-01) | safememset_common_impl + data_copy_vmcheck; VMSUSPEND 恢复 |
| 57 | SYS_PADCONF | `syscall.rs` `dispatch_padconf` | ⚠️ Partial (arm-only) | ARM 32-bit pad configuration; ArmSyscall trait default BadCall; real impl DEFERRED |

**统计** (2026-08-13 更新): 58 个 call_nr (0-57) → 46 个 C 定义 SYS_* + 12 个 reserved/unused (WONTFIX)
- ✅ 完全实现: 28 (FORK/TRACE/KILL/GETKSIG/ENDKSIG/SIGSEND/SIGRETURN/MEMSET/UMAP_REMOTE/VUMAP/IRQCTL/DEVIO/SDEVIO/SETALARM/TIMES/IOPENABLE/SAFECOPYFROM/SAFECOPYTO/VSAFECOPY/SETGRANT/STIME/SETTIME/VTIMER/RUNCTL/GETMCONTEXT/SETMCONTEXT/UPDATE/SCHEDCTL/SCHEDULE/SAFEMEMSET)
- ⚠️ Partial: 10 (EXEC/CLEAR/PRIVCTL/UMAP/VIRCOPY/PHYSCOPY/VDEVIO/GETINFO/ABORT/SPROF/VMCTL/DIAGCTL/EXIT/STATECTL/READBIOS/PADCONF)
- ❌ WONTFIX (reserved/unused): 12 (call_nr 11,12,20,29,30,37,38,41,42,47,48,49 — com.h 无 SYS_* 定义, system.c call_vec[]=NULL)
...
| P1-10 | syscall_device.rs:325-417 | ~~dispatch_devio/vdevio 缺 PortIo trait~~ | ✅ 已修复 (2026-06-14): `PortIo` trait + CHECK_IO_PORT + 对齐检查 + I/O 执行 + DIO 常量映射修复 + 7 个单元测试. **(2026-06-16) PortIo trait 迁移至 minix_plat, x86_64 X86_64PortIo 实现, `CurrentArchSyscall::dispatch_devio` 接入** |
