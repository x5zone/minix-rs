...
fn total_pages(regions: &[BootMemRegion]) -> usize {
    let (tp, _, _) = super::compute_memory_bounds(regions);
    tp
}
...
mod basic {
    ...
    fn segment_tree_alloc_free_basic() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_regions(64)), &make_regions(64));
        let addr = alloc.alloc_mem(4, PageAllocFlags::empty()).unwrap();
        alloc.free_mem(addr, 4);
    }
    ...
    fn buddy_alloc_free_basic() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_regions(64)), &make_regions(64));
        let addr = alloc.alloc_mem(4, PageAllocFlags::empty()).unwrap();
        alloc.free_mem(addr, 4);
    }
    ...
    fn segment_tree_single_page() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_small_regions(1)), &make_small_regions(1));
        let a = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(a.is_ok());
        let b = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(b.is_err());
    ...
    fn buddy_single_page() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_small_regions(1)), &make_small_regions(1));
        let a = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(a.is_ok());
        let b = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(b.is_err());
...
mod exhaustion {
    ...
    fn segment_tree_exhaustion() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_small_regions(4)), &make_small_regions(4));
        let a = alloc.alloc_mem(4, PageAllocFlags::empty());
        assert!(a.is_ok());
        let b = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(b.is_err());
    ...
    fn buddy_exhaustion() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_small_regions(4)), &make_small_regions(4));
        let a = alloc.alloc_mem(4, PageAllocFlags::empty());
        assert!(a.is_ok());
        let b = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(b.is_err());
    ...
    fn segment_tree_alloc_all_then_free_all() {
        let metadata = make_metadata();
        let total = 64;
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_small_regions(total)), &make_small_regions(total));
        let mut addrs = Vec::new();
        for _ in 0..total {
            let addr = alloc.alloc_mem(1, PageAllocFlags::empty()).unwrap();
            addrs.push(addr);
...
mod free_realloc {
    ...
    fn segment_tree_free_realloc() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_small_regions(20)), &make_small_regions(20));
        let a = alloc.alloc_mem(10, PageAllocFlags::empty()).unwrap();
        let b = alloc.alloc_mem(10, PageAllocFlags::empty()).unwrap();
        alloc.free_mem(a, 10);
        let c = alloc.alloc_mem(10, PageAllocFlags::empty()).unwrap();
    ...
    fn buddy_free_realloc() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_small_regions(64)), &make_small_regions(64));
    ...
    fn buddy_merge_on_free() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_small_regions(256)), &make_small_regions(256));
...
mod flags {
    ...
    fn segment_tree_align64k() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_regions(4)), &make_regions(4));
        let addr = alloc.alloc_mem(1, PageAllocFlags::ALIGN64K).unwrap();
        assert_eq!(addr.as_usize() % (64 * 1024), 0);
        alloc.free_mem(addr, 1);
    }
    ...
    fn buddy_align64k() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_regions(4)), &make_regions(4));
        let addr = alloc.alloc_mem(1, PageAllocFlags::ALIGN64K).unwrap();
        assert_eq!(addr.as_usize() % (64 * 1024), 0);
        alloc.free_mem(addr, 1);
    }
    ...
    fn segment_tree_align16k() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_regions(1)), &make_regions(1));
        let addr = alloc.alloc_mem(1, PageAllocFlags::ALIGN16K).unwrap();
        assert_eq!(addr.as_usize() % (16 * 1024), 0);
        alloc.free_mem(addr, 1);
    }
    ...
    fn buddy_align16k() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_regions(1)), &make_regions(1));
        let addr = alloc.alloc_mem(1, PageAllocFlags::ALIGN16K).unwrap();
        assert_eq!(addr.as_usize() % (16 * 1024), 0);
        alloc.free_mem(addr, 1);
    }
    ...
    fn segment_tree_lower1mb() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_regions(2)), &make_regions(2));
        let addr = alloc.alloc_mem(1, PageAllocFlags::LOWER1MB).unwrap();
        assert!(addr.as_usize() < 1024 * 1024);
        alloc.free_mem(addr, 1);
    }
    ...
    fn buddy_lower1mb() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_regions(2)), &make_regions(2));
        let addr = alloc.alloc_mem(1, PageAllocFlags::LOWER1MB).unwrap();
        assert!(addr.as_usize() < 1024 * 1024);
        alloc.free_mem(addr, 1);
    }
...
mod multi_region {
    ...
    fn segment_tree_multi_region() {
        let metadata = make_metadata();
        let mut alloc = SegmentTreeAllocator::init(metadata, total_pages(&make_multi_regions()), &make_multi_regions());
        let a = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(a.is_ok());
        alloc.free_mem(a.unwrap(), 1);
    }
    ...
    fn buddy_multi_region() {
        let metadata = make_metadata();
        let mut alloc = BuddyAllocator::init(metadata, total_pages(&make_multi_regions()), &make_multi_regions());
        let a = alloc.alloc_mem(1, PageAllocFlags::empty());
        assert!(a.is_ok());
        alloc.free_mem(a.unwrap(), 1);
    }
