...
pub(crate) const DIRECT_MAP_BASE: u64 = 0xFFFF_8000_0000_0000;

#[inline]
pub(crate) fn vm_phys_to_virt(phys: PhysBytes) -> VirBytes {
    VirBytes(phys.as_u64() + DIRECT_MAP_BASE)
}
...
pub(crate) fn virt_to_phys(virt: VirBytes) -> PhysBytes {
    PhysBytes::new(virt.0 - DIRECT_MAP_BASE)
}

#[inline]
pub(crate) fn is_direct_map_virt(virt: VirBytes) -> bool {
    virt.0 >= DIRECT_MAP_BASE
}
...
mod tests {
    ...
    fn test_vm_phys_to_virt() {
        ...
        assert_eq!(virt.0, DIRECT_MAP_BASE + 0x1000);
    }
    ...
    fn test_is_direct_map_virt() {
        assert!(is_direct_map_virt(VirBytes(DIRECT_MAP_BASE)));
        assert!(is_direct_map_virt(VirBytes(DIRECT_MAP_BASE + 0x1000)));
