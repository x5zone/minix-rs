//! Test: KernelInfo.memmap reflects the real physical memory layout (aarch64).
//!
...
#[entry]
fn main() -> Status {
    early_console::write_str("### test_memmap (aarch64): verifying UEFI memory map...\n");
    ...
    early_console::write_str("### TEST_RESULT: PASS test-memmap-aarch64 ###\n");
    loop { unsafe { asm!("wfe", options(nomem, nostack)); } }
}
