...
| 字段 | 说明 | Evidence 级别 |
|------|------|-------------|
| **函数名** | C 函数名 + Rust 对应名 | [DIRECT] grep 验证 |
| **语义范围** | 此函数属于哪个模块的什么功能 | [DIRECT] |
| **输入契约** | 参数类型、取值范围、前置条件 | [DIRECT] grep C 签名 |
| **输出契约** | 返回值含义、取值范围 | [DIRECT] grep C 返回路径 |
| **副作用** | 修改的全局状态、IPC 调用、硬件操作 | [MEDIUM] 需读函数体 |
| **错误码** | 所有可能返回的 errno 及触发条件 | [DIRECT] grep error path |
| **时序契约** | 调用顺序约束、并发约束 | [INFERRED] 需分析调用者 |
| **生命周期** | 分配/释放责任、所有权转移 | [MEDIUM] 需追踪 alloc/free |
| **不变量** | 函数执行前后保持的不变量 | [INFERRED] 需分析 |
| **边界条件** | 极端输入、空指针、零长度 | [MEDIUM] 需读边界处理 |
| **架构演进** | 32→64 位变化、类型变化 | [DIRECT] 对比 C 和 Rust |

### 2.2 行为契约表模板

> 对每个核心函数填写此表。非核心函数可简化（仅函数名+输入+输出+错误码）。
...
|------|--------|----------|-------|----------|
| 语义范围 | {模块/功能} | {模块/功能} | ✅/❌ | [DIRECT] |
| 输入契约 | {参数类型/范围/前置条件} | {参数类型/范围/前置条件} | ✅/❌ | [DIRECT] |
| 输出契约 | {返回值含义} | {返回值含义} | ✅/❌ | [DIRECT] |
| 副作用 | {全局状态/IPC/硬件} | {全局状态/IPC/硬件} | ✅/❌ | [MEDIUM] |
| 错误码 | {errno 列表+条件} | {Error 变体+条件} | ✅/❌ | [DIRECT] |
| 时序契约 | {调用顺序约束} | {调用顺序约束} | ✅/❌ | [INFERRED] |
| 生命周期 | {alloc/free 责任} | {RAII/所有权} | ✅/❌ | [MEDIUM] |
| 不变量 | {执行前后不变量} | {执行前后不变量} | ✅/❌ | [INFERRED] |
| 边界条件 | {极端输入处理} | {极端输入处理} | ✅/❌ | [MEDIUM] |
| 架构演进 | {32位行为} | {64位行为} | ✅/❌/N/A | [DIRECT] |
