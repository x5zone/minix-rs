...
> ASCII 图（asc 图）是文档中常用的可视化手段，但滥用或粗制滥造会严重降低文档质量。

- [ ] **必要性**：这个图是否传达了纯文字无法高效表达的信息？如果可以用一两句话说明，就不需要图
- [ ] **对齐精度**：ASCII 图的边框、箭头、文字是否严格对齐？右边界是否参差不齐？
- [ ] **可维护性**：ASCII 图是否过于复杂，导致后续修改困难？
- [ ] **信息密度**：图是否承载了足够的信息量？还是为了"有图"而画图？
...
| 文档 | 表述清晰度 | 章节组织、段落衔接 |
| 文档 | 交叉引用完整性 | 缺少前置文档引用 |
| 文档 | ASCII 图质量 | 滥用、对齐混乱、信息密度低 |
| 代码 | 命名规范 | 不符合 Rust 规范或与 Minix3 不一致 |
| 代码 | 注释覆盖率 | `pub` 函数/类型缺少文档注释、模块缺少模块级文档 |
| 代码 | 注释质量 | 缺少 safety 注释、冗余注释、非英文注释 |
...
> 同一模块（目录）内的文档存在大量共享概念和依赖关系，Review 时必须检查一致性。
...
- [ ] **共享数据结构**：本文档使用的结构体（如 `vmproc`、`vir_region`）是否在同目录其他文档中有更权威的定义？本文档的描述是否与之一致？
- [ ] **共享常量/配置**：本文档引用的常量（如 `CLICK_SIZE`、`VM_RQ_BASE`）是否与同目录其他文档一致？是否有重复定义？
- [ ] **跨模块调用**：本文档描述的 IPC 接口（如 PM → VM）是否与"参见"引用的其他模块文档匹配？
