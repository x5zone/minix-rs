...
1. **Read correct STATE.md path** (tool-isolated; Codex does not share intermediate results with other tools):
   ...
   - **`{doc-stem}`** = target doc basename without extension (e.g. `03-kmain-cstart`). Codex 单 session 无 agent 后缀。
   ...
   - **Auto-init**: `tools/review-init.sh codex {doc-path}` computes `{module}`/`{doc-stem}` and creates standard directories.
2. Output:
```
...
Run coverage-extract.py with full args. `{minix3-module}` = Minix3 module name (vm/pm/kernel/...); `{rw-module}` = rewrite module name (first dir under `notes/rewrite/`). **Codex output paths are hardcoded to `.review/codex/`; do not write to another tool's path.**
```bash
...
python3 tools/coverage-extract/coverage-extract.py {minix3-module} {doc_dir} \
  ...
  --output .review/codex/{rw-module}/scans/{doc-stem}-{agent}-SYMBOLS.md
...
1. Write/update tool-specific STATE.md (tool-isolated; Codex does not share intermediate results):
...
3. **All dimension results → scan.md single file** (NOT 10 dimension check files)
   ...
   - If user explicitly requests another output location, **dual-write**: user-specified path + tool default path (Codex report: `notes/rewrite/{module}/{stage}/{doc-stem}-codex-report.md`).
...
**输出**：写入工具对应的 VERIFY-CHECK.md 路径（Codex: `.review/codex/{module}/VERIFY-CHECK.md`）。VERIFY-CHECK.md 必须包含"验证局限说明"段，标注验证者（同 agent / 跨 agent）及验证方法（grep 重放 / 语义回忆）。
...
- Codex 内允许：跨 agent 轮换（独立 session）
